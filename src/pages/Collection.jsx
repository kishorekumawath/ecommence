...
function Collection() {
  ...
  useEffect(() => {
    ...
    let SelectedCategoryName = reverseSlugToOriginal(categoryName);
    ...
    setSelectedSubCategory([availableSubCategory.find((item) => item.name === reverseSlugToOriginal(subCategoryName))]);
  ...
  const onCategoryToggle = (e) => {
    ...
    setSelectedCategory((prev) => prev.filter((item) => item !== e.target.value));
      ...
      setAvailablesSubCategory((prev)=>prev.filter((item)=> !CollectionsData[e.target.value].some((subCat)=>subCat.name===item.name)));
    } 
  ...
  const onSubCategoryToggle = (e) => {
    ...
    if (selectedSubCategory.some((subCat) => subCat.name === e.target.value)) {
      console.log("unchecking subcategory");
      // remove it from selected subcategory list
      setSelectedSubCategory((prev) => prev.filter((item) => item.name !== e.target.value));
    ...
    // else add it subcategory
      ...
      setSelectedSubCategory((prev) => [...prev, availablesSubCategory.find((item) => item.name === e.target.value)]);
    }
    console.log("Selected Category:", "--", selectedSubCategory);
    
  }

  const reverseSlugToOriginal = (slug) => {
    if (!slug) return ''; // Handle undefined/null case
    return slug
      .split("-")
